/// THIS FILE WAS CREATED BY KARSTEN PEDERSEN FOR OUR 3DGP UNIT.
/// IT SERVES AS A MODEL LOADER.
  ...
  */

  {
    ...
    for(std::vector<Face>::iterator fit = faces.begin();
      fit != faces.end(); fit++)
    {
      pos_buffer.push_back(fit->pa.x); pos_buffer.push_back(fit->pa.y); pos_buffer.push_back(fit->pa.z);
      pos_buffer.push_back(fit->pb.x); pos_buffer.push_back(fit->pb.y); pos_buffer.push_back(fit->pb.z);
      pos_buffer.push_back(fit->pc.x); pos_buffer.push_back(fit->pc.y); pos_buffer.push_back(fit->pc.z);
    }
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(pos_buffer.at(0)) * pos_buffer.size(), &pos_buffer.at(0),
      GL_STATIC_DRAW);
  ...
  if(tcs.size() > 0)
  {
    ...
    for(std::vector<Face>::iterator fit = faces.begin();
      fit != faces.end(); fit++)
    {
      tb.push_back(fit->tca.x); tb.push_back(fit->tca.y);
      tb.push_back(fit->tcb.x); tb.push_back(fit->tcb.y);
      tb.push_back(fit->tcc.x); tb.push_back(fit->tcc.y);
    }
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(tb.at(0)) * tb.size(), &tb.at(0),
      GL_STATIC_DRAW);
  ...
  */
  if(normals.size() > 0)
  {
    ...
    for(std::vector<Face>::iterator fit = faces.begin();
      fit != faces.end(); fit++)
    {
      nb.push_back(fit->na.x); nb.push_back(fit->na.y); nb.push_back(fit->na.z);
      nb.push_back(fit->nb.x); nb.push_back(fit->nb.y); nb.push_back(fit->nb.z);
      nb.push_back(fit->nc.x); nb.push_back(fit->nc.y); nb.push_back(fit->nc.z);
    }
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(nb.at(0)) * nb.size(), &nb.at(0),
      GL_STATIC_DRAW);
